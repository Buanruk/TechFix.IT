...
$regexMap = [
    ...
    'tv'      => '(tv|ทีวี|monitor|json)',
];

/* ===== Helper ===== */
function h($s){ return htmlspecialchars((string)$s, ENT_QUOTES, 'UTF-8'); }

// ===== [แก้ไข] เพิ่ม $searchQuery เข้ามาในฟังก์ชัน =====
...
// [แก้ไข] ส่ง $searchQuery เข้าไปด้วย
[$whereSQLCnt, $typesCnt, $valsCnt] = build_where_and_params($filterStatus, $filterDtype, $regexMap, $searchQuery);
...
// ===== [แก้ไข] เพิ่ม $searchQuery เข้ามาในฟังก์ชัน =====
function page_url($p, $status, $dtype, $searchQuery){
    // เพิ่ม &q=... เข้าไปใน URL
    return '?status='.urlencode($status).'&dtype='.urlencode($dtype).'&q='.urlencode($searchQuery).'&page='.(int)max(1,$p);
}
?>
<!DOCTYPE html>
<html lang="th">
<head>
    ...
    <style>
        ...
        /* ===== [เพิ่มใหม่] CSS สำหรับช่องค้นหาและปุ่ม ===== */
        .search-input {
            appearance:none;height:42px;line-height:42px;padding:0 14px;min-width:240px;
            border:1px solid #e6effa;border-radius:12px;background:#fff;font-size:15px;font-weight:700;color:#1f2937;outline:none;
            box-shadow:0 8px 18px rgba(10,37,64,.06), inset 0 1px 0 rgba(255,255,255,.6);
        }
        .search-input:focus {
        ...
        .btn-search {
            ...
            border:1px solid #0d47a1; border-radius:12px; cursor:pointer;
            background: #0d47a1; color: #fff;
            transition:all .18s ease;
        }
        .btn-search:hover { background: #113761; border-color: #113761; }
        /* ===== [จบส่วนเพิ่มใหม่] ===== */
        ...
        @media (max-width:860px){
            /* [แก้ไข] เปลี่ยนเป็น center เพื่อให้ 3 รายการอยู่ตรงกลาง */
        ...
        @media (max-width:640px){
            ...
            /* ฟอร์มกรองซ้อนเป็นแถว (เพิ่ม search-input) */
            .toolbar{flex-direction:column;align-items:stretch}
            .toolbar .group{width:100%;justify-content:space-between}
            .toolbar .select{min-width:unset;flex:1}
            .search-input{min-width:unset;flex:1} /* [เพิ่มใหม่] */
            .btn-search{padding:0 14px} /* [เพิ่มใหม่] */
    ...
    <section class="card">
        ...
        <nav class="pager" aria-label="เปลี่ยนหน้า">
            <?php
                ...
                $next = $page + 1;
            ?>
            <a class="<?= $page<=1 ? 'disabled':'' ?>" href="<?= $page<=1 ? '#' : h(page_url($prev, $filterStatus, $filterDtype, $searchQuery)) ?>" aria-label="ก่อนหน้า">«</a>
            ...
            <?php
                ...
                if ($start > 1){
                    echo '<a href="'.h(page_url(1,$filterStatus,$filterDtype,$searchQuery)).'">1</a>';
                    if ($start > 2) echo '<span class="disabled">…</span>';
                }
                for($p=$start; $p<=$end; $p++){
                    if ($p == $page) echo '<span class="active">'.$p.'</span>';
                    else echo '<a href="'.h(page_url($p,$filterStatus,$filterDtype,$searchQuery)).'">'.$p.'</a>';
                }
                if ($end < $totalPages){
                    if ($end < $totalPages-1) echo '<span class="disabled">…</span>';
                    echo '<a href="'.h(page_url($totalPages,$filterStatus,$filterDtype,$searchQuery)).'">'.$totalPages.'</a>';
                }
            ?>

            <a class="<?= $page>=$totalPages ? 'disabled':'' ?>" href="<?= $page>=$totalPages ? '#' : h(page_url($next, $filterStatus, $filterDtype, $searchQuery)) ?>" aria-label="ถัดไป">»</a>
...
<script>
    ...
    async function refreshTickets(){
        try{
            ...
            url.searchParams.set('poll', 'status'); // ขอเฉพาะ id/status (ยังคงพารามิเตอร์ dtype/status/q/page ปัจจุบัน)
